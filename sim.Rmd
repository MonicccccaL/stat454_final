...
>>>>>>> e83188852ca72727a8b01b0fb87c0efee765b01e
...
=======
This dataset contains housing data of 464 Hennepin and Ramsey census tracts including median prices of housing in USD (`HouseValueE`), median household income in the past 12 months (in 2019 inflation) (`IncomeE`), and average household size of occupied houses (`HouseholdSizeE`). We create the variable called `vble` with the logarithm of the median prices, and map the log price using **mapview** (Figure 1). The map suggests that the housing prices are greater in the southwest, and prices are related to those in neighboring areas. Noted that the empty area is the University of Minnesota area, since there is no houses listed.
>>>>>>> e83188852ca72727a8b01b0fb87c0efee765b01e
...
=======
origin <- mapview(mn_data, zcol = "vble")
origin
>>>>>>> e83188852ca72727a8b01b0fb87c0efee765b01e
```
...
\mu_i=\beta_0+\beta_1 \times Income+\beta_2 \times HouseholdSize + u_i+ε_i
$$
where, (Question? does this mean that each household has a seperate intercept) - $\beta_0$: the global intercept, which every household will have the same intercept. - $\beta_1$, $\beta_2$: represent respectively, the coefficients of the covariates household income and household size. - $\mu_i$: a spatially structured effect modeled with a CAR structure,$\mu_i| u_{-i} \sim N(\mu_{\delta_i}, \frac{\sigma^2_u}{n_{\delta_i}})$. - $ε_i$: an unstructured effect modeled as $ε _i \sim N(\theta, \sigma^2_ε )$.
...
In R-INLA, the BYM model can also be specified with `model = "bym"` and this comprises both the spatial and unstructured components. Alternatively, we can use the BYM2 model (Simpson et al. 2017) which is a reparametrization of the BYM model that uses a scaled spatial component $u_∗$ and an unstructured component $ε_∗$, which allows for separate inference on the spatial and unstructured components. The formula of the model using the BYM2 component is specified as follows:
