...
-   $x$: represents the latent Gaussian field, which the distribution is
    assumed to be Gaussian Markov random field (GMRF)
...
$$
f(y|x,\theta)=\prod_{i \in \mathcal {I}} f(y_i|\eta_i, \theta)
$$
...
$$
f(x,\theta|y)\propto f(θ)f(x|\theta) \prod_{i \in \mathcal {I}} f(y_i|x_i, \theta)\\
\propto f(\theta)|Q(\theta)|^{\frac{1}{2}} exp \{ −\frac{1}{2}x^\top Q(\theta)x + \sum_{i \in \mathcal{I}}log(f(y_i|x_i,\theta)) \}.
$$
