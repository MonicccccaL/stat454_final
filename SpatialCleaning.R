...
mn_quant <- get_acs(state = "MN", county = c("Hennepin","Ramsey"), geography = "tract", 
                        variables = VARS, geometry = TRUE, output='wide')

mn_quant <- mn_quant %>% select(-ends_with('M'))

#BirthPlace: B06001_001
#Industry: 	C24070_001
#Renter/Owner: B25010_001
#Race: B02001_001
get_mn_data <- function(V,V1,NAMES){
  mn_cat <- get_acs(state = "MN", county = c("Hennepin","Ramsey"), geography = "tract",
  ...
  mn_cat %>%
    ...
    mutate(across(-c(GEOID,NAME,summary_est,summary_moe), ~.x/mn_cat$summary_est)) %>%
...
industry = get_ny_data(V,V1,NAMES)
...
NAMES <- v19 %>% filter(name %in% V) %>% pull(label) %>% str_replace('Estimate\\!\\!Total\\:\\!\\!','')
birthplace = get_ny_data(V,V1,NAMES)
...
NAMES <- v19 %>% filter(name %in% V) %>% pull(label) %>% str_replace('Estimate\\!\\!Total\\:\\!\\!','')
housetype = get_mn_data(V,V1,NAMES)
...
# 
# ny_data <- st_transform(ny_data,crs = 6505)
# ny_data$AREA = st_area(ny_data) %>% as.vector()
...
# roads_sub <- st_crop(roads_sub,st_bbox(ny_data))
# 
...
# 
mn_data <- mn_data %>% filter(!is.na(HouseValueE))
# 
CodeBook <- v19 %>% filter(name %in% NMS)
# 
save(mn_data,CodeBook,file = 'SpatialData.RData')
# 
# 
# 
# 
