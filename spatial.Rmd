...
\text{ where } \mu_i = \beta_0 + \sum_{k=1}^k x_{ik} \beta_k + ø_i + \epsilon_i \\
&& \\
...
$\beta_0 + x_{ik}\beta_k$ and random effect, $ø_i + \epsilon_i$. For the
fixed effect, $\beta_0$ is the intercept, $\beta_k$ is a set of
coefficients of k predictors we are interested in. The random effect
includes spatial random effect $ø_i$ and non-spatial random effect
$\epsilon_i$. Spatial random effect $ø_i$ accounts for the spatial
...
spatial random effect $ø_i$ is modeled by the intrinsic conditional
...
effects $ø = (ø_1, ø_2, …, ø_N)$, $N$ represents the number of regions
we are looking at. The distribution of ø_i is determined by the sum of
weighted values of its neighbors $w_{ij}ø_j$. The model is specified as
$$u_i | u_j, j ≠ i \sim N(\sum_{j \in N_i} w_{ij}u_j, \sigma_i^2)$$ In
the model, $j$ is the neighbor region in the neighborhood set of region
$i$, $N_i$. $n$ is the count of neighbors in the set $N_i$, and
$\sigma_i$ is the unknown variance of random effect $u_i$.

Besag (1974) proved that the joint distribution of $u$ is a multivariate
normal centered at 0. The variance of $ø$ is specified as a precision
matrix $Q$ which is the inverse of a covariance matrix. With that,
$$u \sim N(0, Q^{-1})$$ For the multivariate normal random variable $ø$,
the precision matrix $Q$ is constructed from two matrices: diagonal
matrix $D$ and adjacency matrix, which is the neighborhood matrix we
defined above $W$. For CAR model, $Q$ is defined as
...
model, we set $\alpha$ is 1 and the above equation can be simplified as
$$Q = D - W$$
...
$\sigma_i^2 /n$ implies that the variance of spatial effect decreases as
the number of neighbors increases. In other words, if the region has
more neighbors, we are more confident in estimating its spatial effect.
...
model, the BYM2 model, described by Riebler et al. (2016) and Simpson et
